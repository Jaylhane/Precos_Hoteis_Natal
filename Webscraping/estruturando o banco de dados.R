...
View(quarto_i)
#ObservaÃ§Ãµes dos dados baixados:
  
#1) deu erro de time out do site no dia 24/02/2020, os dados foram baixados apenas atÃ© dia 11/12/2020;
...
#3)coloquei os dados para baixar a partir de 20h, mas nÃ£o conseguiu completar atÃ© 00h35, dia 26/02/2020 os dados foram baixados atÃ© 01/08/2020. estÃ¡ gastando muito tempo para baixar pois estÃ¡ verificando 10 pÃ¡ginas, sendo que em alguns dias o nÃºmero de pÃ¡ginas Ã© menor e ao nÃ£o encontrar o offset dÃ¡ erro. NecessÃ¡rio pensar em uma forma de otmizar esse processo e verificar o nÃºmero de pÃ¡ginas. Removi a suspensao de 3 seg ao final do ciclo do dia para ver se otimiza o tempo e diminui para 2 dentro do ciclo da pÃ¡gina, e nao resolveu, irei gerar um laÃ§o para verificar o nÃºmero de pÃ¡ginas : .sr_pagination_link

#27/02 , baixou atÃ© 28/08/2020 - Error in read_connection_(x, n) : 
#Evaluation error: Failure when receiving data from the peer.
#mandei continuar baixando a partir de 28/08/2019 para tentar mesclar o banco de dados: 23h58, vamos ver atÃ© quando vai conseguir baixar. - nÃ£o parou as 00h35 - baixou apenas ate 24/10/2020 na pg 8

#28/02, baixou atÃ© pg 3 11/08/2020 - Error in open.connection(x, "rb") : HTTP error 502. Inclui um laÃ§o para encerrar todas as conexÃµes apÃ³s 120 dias e esperar 5 minutos antes de continuar

#29/02, as alteraÃ§Ãµes no codigo permitiram baixar 496 dias (ate 08/07/2021). apareceu um novo erro relacionado ao nÃºmero de pÃ¡ginas: " Error in seq.default(0, qtd, 25) : 'to' deve ter comprimento 1" , acredito que por nÃ£o haver informaÃ§Ãµes na pÃ¡gina ele nÃ£o conseguiu prosseguir baixando. procurar uma forma de quando dar esse tipo de erro encerrar todas as conexÃµes e salvar os dados. inclui um ifelse para caso a qtd de propriedades for menor de 26 estabelecer a qtd como 0 e se propriedades for igual a zero fechar conexÃµes e salvar o arquivo. 
...
#parou em 13/04/2021 - salvei e mandei continuar baixando . o laÃ§o para verificar se chegou a 0 propriedades nÃ£o funcionou, propriedades retornou como numeric (empty) e apareceu a seguinte mensagem: Error in if (propriedades == 0) { : argument is of length zero. baixou 502 dias com os incrementos no codigo. foi necessario intervir para continuar baixando, o erro de perda de conexÃ£o persiste. 

#02/03 - apareceu apenas o erro de propriedades == 0, sem falhas de conexÃ£o. 

#04/03 - computador desligou antes de baixar tudo, tentei continuar baixando os demais dias para continuar, talvez seja necessÃ¡rio fazer uma limpeza no dia 04/11/2020 deste dia
